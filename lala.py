...
model = NeuralNetwork()
batch_size = 128 #
learning_rate = 0.0001 # lower lr  TEST ON THE 0.2 PARTITION START
epochs = 65  #did 100
losses = []
# ADD THE  SCHEDULER 
...
# port_counts = set_cleaned['Destination Port'].value_counts()

# # Print the counts
# top_n = 20
# print(f"Top {top_n} most frequent destination ports:")
# print(port_counts.head(top_n))
...
#     factor=0.5,       # Reduce LR by half each time
...
# Calibrate before conversion
#print("\n=== Calibrating QAT Model ===")
#qat_model = calibrate_qat_model(qat_model, train_loader, torch.device('cpu'), num_batches=100)
...
def export_fixed_point(model, filename="nn_params.bpf.h", scale_bits=16):
    model.eval()
    Q = 1 << 16  
...
#export_fixed_point(quantized_model)
