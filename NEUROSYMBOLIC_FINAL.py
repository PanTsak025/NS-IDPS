...
class NeuralNetwork(nn.Module):                                                     
    def __init__(self, input_size=13, hidden_size=13, output_size=2):
        ...
        self.layers = nn.Sequential(
            nn.Linear(input_size, hidden_size, bias= False),  # Layer 1: 32 → 32
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size,bias= False),  # Layer 2: 32 → 32
            nn.ReLU(),
            nn.Linear(hidden_size, output_size,bias= False)   # Layer 3: 32 → 2
        )
    def forward(self, x):
